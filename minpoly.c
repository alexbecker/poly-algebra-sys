// minpoly.c
// finds the minimal polynomial of a low-degree real algebraic number
...
// given a POSITIVE value x and integers k and deg
...
static subset_sum_problem *to_subset_sum(root_type x, int k, int deg) {
	subset_sum_problem *problem = (subset_sum_problem*) malloc(sizeof(subset_sum_problem));
	problem->size = k * (deg + 1);
	problem->items = (root_type*) malloc(sizeof(root_type) * problem->size);
	...
	problem->sum = (root_type) -1;
	for (int i=1; i<=deg; i++) {
		problem->sum += ((root_type) pow2(k-1)) * ((root_type) pow((double) x, (double) i));
	}
	// items
	for (int i=0; i<=deg; i++) {
		for (int j=0; j<k; j++) {
			problem->items[k * i + j] = (root_type) ((double) pow2(j) * pow((double) x, (double) i)); 
		}
	}
...
// NOTE: only works for positive values of b.center
static polynomial *get_min_poly_pos(ball b, int max_k, int max_deg) {
	root_type error;
	polynomial *min_poly;
...
// extends get_min_poly_pos to all real values of b.center
// also handles the case where b contains 1
polynomial *get_min_poly(ball b, int max_k, int max_deg) {
	if (test_ball_membership(1, b)) {
		polynomial *result = alloc_polynomial(1);
		result->coefficients[0] = 1;
		result->coefficients[1] = -1;	
		return result;
	}
	if (b.center > 0)
		return get_min_poly_pos(b, max_k, max_deg);
	b.center *= -1;
	polynomial *temp = get_min_poly_pos(b, max_k, max_deg);
	polynomial *result = linear_change_of_variables(*temp, -1, 0);
...
void test_to_subset_sum() {
	...
	subset_sum_problem *problem = to_subset_sum(ROOT_2, 3, 2);
	printf("sum: %lf\nitems: ", problem->sum);
